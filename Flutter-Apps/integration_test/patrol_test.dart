...
void main() {
  ...
  group('Simple Flutter App Integration Tests', () {
    testWidgets('App loads and displays all main components', (tester) async {
      ...
      // Test 8: Test navigation between tabs
      final exploreNav = find.text('Explore');
      if (exploreNav.evaluate().isNotEmpty) {
        await tester.tap(exploreNav);
        await tester.pumpAndSettle();
      }

      final favoritesNav = find.text('Favorites');
      if (favoritesNav.evaluate().isNotEmpty) {
        await tester.tap(favoritesNav);
        await tester.pumpAndSettle();
      }
